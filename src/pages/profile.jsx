...
import { ProfileCard, ProfileSkeleton, PostCard, ErrorComp } from "../components/index.js";
...
function Profile() {
  ...
  const { isPending, error, data, isLoading, isError } = useQuery({
    queryKey: ["profile"],
    queryFn: () => userService.getUserDetails(user_id)
      .then(res => {
  ...
  const { isLoading: isPostLoading, data: userPost, isError: isPostError } = useQuery({
    queryKey: ["userPost"],
    queryFn: () => postService.getUserPosts(user_id)
      .then(res => res.data)
  ...
  return (
    ...
    smm:w-[70%] sm:w-[60%] md:w-[55%] lg:w-[40%]">
      ...
      <div id="post-container" class="sm:w-70% h-full rounded-sm mx-auto flex flex-col flex-grow items-center gap-4 
        w-full px-6">
        {isPostLoading ? (
          <>
            <PostSkeleton />
